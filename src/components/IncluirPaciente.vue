    ...
    <div class="card">
      ...
      <div class="card-body">
        ...
        <div class="alert alert-sucess" role="alert" v-if="!error">
          Paciente cadastrado com sucesso {{msg}}
        </div>
        <form @submit.stop.prevent="salvar">
          ...
          <button type="submit" class="btn btn-primary" ref="meu modal">Cadastrar</button>
        </form>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async salvar() {
      ...
      try {
        const res = await api.post("pacientes", this.paciente, header);
